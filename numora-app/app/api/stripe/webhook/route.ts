 ...
 * Etapa "Stripe 5.4A — Webhook Foundation" — recebe eventos do Stripe TEST
 * MODE, verifica a assinatura e registra de forma idempotente.
 ...
 * — quem chama é o Stripe, não um usuário logado. A única autenticação
 * válida aqui é a assinatura Stripe (`stripe-signature` + `STRIPE_WEBHOOK_SECRET`).
 * `user_id`/`numora_user_id` só aparecem dentro do PAYLOAD do evento (ex.:
 * `metadata.numora_user_id` de uma Checkout Session, Stripe 5.3) — nunca
 * são confiados sem a assinatura já ter sido validada primeiro.
 *
